...
地理オブジェクトの所属処理まで追跡した結果、次の構造が確認された。
...
- `FUN_1409c32d0`：`state.cpp` 由来。Provinceの既存Stateを検査した後、`province + 0xC0 = thisState` 相当の代入を行う。
- `FUN_140deb2a0`：`strategicregion.cpp` 由来。`"Province has added another strategic region as theirs..."` という警告で旧・新Strategic Regionを扱い、最後に `province + 0xC8 = thisStrategicRegion` 相当の代入を行う。警告内では両Strategic Regionの `+0x58` が識別値として表示される。
- `FUN_140f23c00`：任務対象Provinceと基地・空母側ProvinceのStrategic Region識別値を比較し、航空能力へmodifierを適用する。

実際の航空能力計算は、意味的には次の構造だった。
...
**基地・空母と任務対象が同一Strategic Regionなら `air_home_defence_factor`、別Strategic Regionなら `air_power_projection_factor` が選択され、どちらもAir AttackとAir Defenceのfactorへ同じ経路で加算される。Agilityには加算されない。**

これはState一致判定ではない。`CProvince + 0xC0 = CState*` と `CProvince + 0xC8 = CStrategicRegion*` が別フィールドとして直接確認され、航空計算は後者の `+0x58` を比較している。

証拠レベルは次のように記録する。

- **Confirmed**：上記offsetのポインタ型、同一 / 別Strategic Regionによるmodifier選択、Air Attack / Air Defenceへの加算、Agilityへの非加算。
- **Strong inference**：`CStrategicRegion + 0x58` は実質的なStrategic Region ID。識別値としての役割はコードから明確だが、PDBがないため正式なC++メンバ名そのものは未確認。
- **Unknown**：このmodifierが別のゲームシステムでも追加利用されるか。
