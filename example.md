...
HOI4内部では、
「home判定ではない場合に、Air Attack と Air Defence のfactorへ加算されるmodifier」
として処理されています。
今回の解析で確認できた対応は、
...
0x74 = 116	air_home_defence_factor	特殊補正	home側
0x75 = 117	air_power_projection_factor	特殊補正	away側
...
CShip
へのRTTI Castもあります。
つまり空母艦載機についても考慮されています。
したがって、
Power Projectionは「homeではない場所で作戦する航空隊」に関係する

という理解は、少なくとも内部コードの分岐と一致します。
ただし、
未解決
iVar2
iVar5
...
そこまでは今回追っていません。
なので厳密には、
「同じ内部location IDならHome Defence、違うならPower Projection」
...
iVar2 != iVar5
が「away」。
分岐先modifier名そのものがHome Defence / Power Projectionなので、意味としてはほぼ間違いないですが、比較しているIDの正式な型・名前は未特定。
今回は未確認
...
なので今回の最終的な成果は、
air_power_projection_factor は、homeではない側の航空隊についてAir AttackとAir Defenceのfactorを強化する。Agilityは強化しない。
...
を見つけて、その後 0x6f / 0x70 / 0x71 をそれぞれ Attack / Defence / Agility まで逆引きできたところが、今回の解析で一番決定的な部分です。